...
class InvertedIndex:
    ...
    def __enter__(self):
        ...
        with open(self.metadata_file_path, 'rb') as f:
            ...
            data = pickle.load(f)
            if len(data) == 4:
                self.postings_dict, self.terms, self.doc_length, self.avg_doc_len = data
            else:
                self.postings_dict, self.terms, self.doc_length = data
                self.avg_doc_len = sum(self.doc_length.values()) / len(self.doc_length) if self.doc_length else 0.0
        self.term_iter = self.terms.__iter__()
        return self
    ...
    def __exit__(self, exception_type, exception_value, traceback):
        ...
        if self.doc_length:
            self.avg_doc_len = sum(self.doc_length.values()) / len(self.doc_length)
        else:
            self.avg_doc_len = 0.0
        # Menutup index file
